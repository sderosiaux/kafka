...
import kafka.log.cleaner.{CleanerConfig, LogCleaner}
...
class LogManager(
  ...
  @volatile private var recoveryPointCheckpoints = liveLogDirs
    .map(
      dir =>
        (dir,
         new OffsetCheckpointFile(new File(dir, RecoveryPointCheckpointFile),
  ...
  @volatile private var logStartOffsetCheckpoints = liveLogDirs
    .map(
      dir =>
        (dir,
         new OffsetCheckpointFile(new File(dir, LogStartOffsetCheckpointFile),
                                  logDirFailureChannel)))
    .toMap
  ...
  /**
    ...
    * </ol>
    */
  private def createAndValidateLogDirs(
      dirs: Seq[File],
      ...
      try {
        if (initialOfflineDirs.contains(dir))
          throw new IOException(
            s"Failed to load ${dir.getAbsolutePath} during broker startup")
        ...
        if (!dir.exists) {
          ...
          if (!created)
            throw new IOException(
              s"Failed to create data directory ${dir.getAbsolutePath}")
        }
        if (!dir.isDirectory || !dir.canRead)
          throw new IOException(
            s"${dir.getAbsolutePath} is not a readable log directory.")
        ...
        if (!canonicalPaths.add(dir.getCanonicalPath))
          throw new KafkaException(
            s"Duplicate log directory found: ${dirs.mkString(", ")}")
  ...
  def resizeRecoveryThreadPool(newSize: Int): Unit = {
    info(
      s"Resizing recovery thread pool size for each data dir from $numRecoveryThreadsPerDataDir to $newSize")
  ...
  def handleLogDirFailure(dir: String) {
    ...
    logCreationOrDeletionLock synchronized {
      ...
      if (_liveLogDirs.isEmpty) {
        fatal(
          s"Shutdown broker because all log dirs in ${logDirs.mkString(", ")} have failed")
        Exit.halt(1)
      }

      recoveryPointCheckpoints = recoveryPointCheckpoints.filter {
        case (file, _) => file.getAbsolutePath != dir
      }
      logStartOffsetCheckpoints = logStartOffsetCheckpoints.filter {
        case (file, _) => file.getAbsolutePath != dir
      }
      ...
      info(
        s"Logs for partitions ${offlineCurrentTopicPartitions.mkString(",")} are offline and " +
          s"logs for future partitions ${offlineFutureTopicPartitions.mkString(
            ",")} are offline due to failure on log directory $dir")
      dirLocks
        .filter(_.file.getParent == dir)
        .foreach(dir => CoreUtils.swallow(dir.destroy(), this))
    }
  }
    ...
    dirs.flatMap { dir =>
      try {
        ...
        case e: IOException =>
          logDirFailureChannel.maybeAddOfflineLogDir(
            dir.getAbsolutePath,
            s"Disk error while locking directory $dir",
            e)
          None
      }
    }
  ...
  private def loadLog(logDir: File,
    ...
    val config =
      topicConfigs.getOrElse(topicPartition.topic, currentDefaultConfig)
      ...
      if (previous != null) {
        if (log.isFuture)
          throw new IllegalStateException(
            "Duplicate log directories found: %s, %s!"
              .format(log.dir.getAbsolutePath, previous.dir.getAbsolutePath))
        else
          throw new IllegalStateException(
  ...
  private def loadLogs(): Unit = {
    ...
    for (dir <- liveLogDirs) {
      try {
        ...
        if (cleanShutdownFile.exists) {
          debug(
            s"Found clean shutdown file. Skipping recovery for all logs in data directory: ${dir.getAbsolutePath}")
        } else {
        ...
        try {
          ...
          case e: Exception =>
            warn(
              s"Error occurred while reading recovery-point-offset-checkpoint file of directory $dir",
              e)
        ...
        try {
          ...
          case e: Exception =>
            warn(
              s"Error occurred while reading log-start-offset-checkpoint file of directory $dir",
              e)
        }
    ...
    try {
      for ((cleanShutdownFile, dirJobs) <- jobs) {
        ...
        try {
          ...
          case e: IOException =>
            ...
            error(
              s"Error while deleting the clean shutdown file $cleanShutdownFile",
              e)
        }
      }
      ...
      case e: ExecutionException =>
        error(
          s"There was an error in one of the threads during logs loading: ${e.getCause}")
        throw e.getCause
    } finally {
  ...
  def startup() {
    ...
    if (scheduler != null) {
      info(
        "Starting log cleanup with a period of %d ms.".format(retentionCheckMs))
  ...
  def shutdown() {
    ...
    for (dir <- logDirs) {
      removeMetric("LogDirectoryOffline",
                   Map("logDirectory" -> dir.getAbsolutePath))
    }
    ...
    try {
      for ((dir, dirJobs) <- jobs) {
        ...
        CoreUtils.swallow(
          Files.createFile(new File(dir, Log.CleanShutdownFile).toPath),
          this)
      }
    } catch {
      case e: ExecutionException =>
        error(
          s"There was an error in one of the threads during LogManager shutdown: ${e.getCause}")
        throw e.getCause
    } finally {
      threadPools.foreach(_.shutdown())
  ...
  /**
    ...
    */
  // Only for testing
  private[log] def checkpointRecoveryOffsetsAndCleanSnapshot(
      dir: File,
      logsToCleanSnapshot: Seq[Log]): Unit = {
    try {
      checkpointLogRecoveryOffsetsInDir(dir)
      logsToCleanSnapshot.foreach(
        _.deleteSnapshotsAfterRecoveryPointCheckpoint())
    } catch {
      case e: IOException =>
        logDirFailureChannel.maybeAddOfflineLogDir(
          dir.getAbsolutePath,
  ...
  def maybeUpdatePreferredLogDir(topicPartition: TopicPartition,
                                 logDir: String): Unit = {
    ...
    if (!getLog(topicPartition).exists(_.dir.getParent == logDir) &&
        !getLog(topicPartition, isFuture = true)
          .exists(_.dir.getParent == logDir))
  ...
  /**
    ...
    */
  def getLog(topicPartition: TopicPartition,
      ...
      getLog(topicPartition, isFuture).getOrElse {
        ...
        if (!isNew && offlineLogDirs.nonEmpty)
          throw new KafkaStorageException(
            s"Can not create log for $topicPartition because log directories ${offlineLogDirs
              .mkString(",")} are offline")
        ...
        try {
          ...
          case e: IOException =>
            val msg =
              s"Error while creating log for $topicPartition in dir $logDir"
    ...
    try {
      def nextDeleteDelayMs: Long = {
        if (!logsToBeDeleted.isEmpty) {
          ...
          scheduleTimeMs + currentDefaultConfig.fileDeleteDelayMs - time
            .milliseconds()
        } else
      ...
      while ({ nextDelayMs = nextDeleteDelayMs; nextDelayMs <= 0 }) {
        ...
        if (removedLog != null) {
          try {
            removedLog.delete()
            info(
              s"Deleted log for partition ${removedLog.topicPartition} in ${removedLog.dir.getAbsolutePath}.")
          } catch {
            case e: KafkaStorageException =>
              error(
                s"Exception while deleting $removedLog in dir ${removedLog.dir.getParent}.",
                e)
          }
        }
      }
      ...
      try {
        ...
        case e: Throwable =>
          if (scheduler.isStarted) {
            ...
            error(s"Failed to schedule next delete in kafka-delete-logs thread",
                  e)
          }
      }
    }
  ...
  def replaceCurrentWithFutureLog(topicPartition: TopicPartition): Unit = {
    logCreationOrDeletionLock synchronized {
      ...
      if (sourceLog == null)
        throw new KafkaStorageException(
          s"The current replica for $topicPartition is offline")
      if (destLog == null)
        throw new KafkaStorageException(
          s"The future replica for $topicPartition is offline")
      ...
      if (cleaner != null) {
        cleaner.alterCheckpointDir(topicPartition,
                                   sourceLog.dir.getParentFile,
                                   destLog.dir.getParentFile)
        cleaner.resumeCleaning(Seq(topicPartition))
        info(s"Compaction for partition $topicPartition is resumed")
      }
      ...
      try {
        ...
        sourceLog.close()
        checkpointRecoveryOffsetsAndCleanSnapshot(sourceLog.dir.getParentFile,
                                                  ArrayBuffer.empty)
      ...
      info(
        s"The current replica is successfully replaced with the future replica for $topicPartition")
    }
  }
  ...
  def asyncDelete(topicPartition: TopicPartition,
    ...
    if (removedLog != null) {
      ...
      if (cleaner.isDefined && !isFuture) {
        cleaner.foreach(_.abortCleaning(topicPartition))
        cleaner.foreach(_.updateCheckpoints(removedLog.dir.getParentFile))
      }
      removedLog.renameDir(Log.logDeleteDirName(topicPartition))
      checkpointRecoveryOffsetsAndCleanSnapshot(removedLog.dir.getParentFile,
                                                ArrayBuffer.empty)
      ...
      throw new KafkaStorageException(
        s"Failed to delete log for ${if (isFuture) "future" else ""} $topicPartition because it may be in one of the offline directories ${offlineLogDirs
          .mkString(",")}")
    }
    removedLog
  }
  ...
  def cleanupLogs() {
    ...
    val deletableLogs = cleaner
      .map(_.pauseCleaningForNonCompactedPartitions())
      .getOrElse(currentLogs.filter {
    ...
    try {
      ...
      cleaner.foreach(_.resumeCleaning(deletableLogs.map(_._1)))
    }

    debug(
      s"Log cleanup completed. $total files deleted in " +
  ...
  def isLogDirOnline(logDir: String): Boolean = {
    ...
    if (!logDirs.exists(_.getAbsolutePath == logDir))
      throw new LogDirNotFoundException(
        s"Log dir $logDir is not found in the config.")
  ...
  private def flushDirtyLogs(): Unit = {
    ...
    for ((topicPartition, log) <- currentLogs.toList ++ futureLogs.toList) {
      try {
        ...
        debug(
          s"Checking if flush is needed on ${topicPartition.topic} flush interval ${log.config.flushMs}" +
